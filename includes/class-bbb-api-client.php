...
class BBB_Api_Client {
	...
	public function get_tabelle( int $liga_id ): array|WP_Error {
		...
		return array(
			'liga_data' => $response['data']['ligaData'] ?? array(),
			'entries'   => $response['data']['tabelle']['entries'] ?? array(),
		);
	}
	...
	public function get_liga_matchday( int $liga_id, int $matchday = 1 ): array|WP_Error {
		...
		return array(
			'liga_data' => $data['ligaData'] ?? array(),
			'spieltage' => $data['spieltage'] ?? array(),
	...
	public function get_tournament_rounds( int $liga_id ): array|WP_Error {
		...
		$spieltage = $first['spieltage'] ?? array();
		$liga_data = $first['liga_data'] ?? array();

		$rounds = array(
			1 => array(
				'name'    => $spieltage[0]['bezeichnung'] ?? '1. Runde',
				'matches' => $first['matches'] ?? array(),
			),
		);
	...
	public function get_liga_spielplan( int $liga_id ): array|WP_Error {
		...
		return array(
			'liga_data' => $response['data']['ligaData'] ?? array(),
			'matches'   => $response['data']['matches'] ?? array(),
		);
	}
	...
	public function get_club_team_ids( int $club_id, int $range_days = 150 ): array {
		...
		foreach ( $matches as $match ) {
			foreach ( array( 'homeTeam', 'guestTeam' ) as $side ) {
				$team = $match[ $side ] ?? array();
	...
	public function get_club_teams_detailed( int $club_id, int $range_days = 150 ): array {
		...
		foreach ( $matches as $match ) {
			$liga_data = $match['ligaData'] ?? array();
	...
	private function handle_response( $response ): array|WP_Error {
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error( 'bbb_api_http_error', "HTTP {$code}", array( 'status' => $code ) );
		}
	...
	private function is_near_timeout( int $buffer_seconds = 5 ): bool {
		...
		// (float) cast fully sanitizes; no unslashing needed for a numeric value.
		$start   = isset( $_SERVER['REQUEST_TIME_FLOAT'] ) ? (float) $_SERVER['REQUEST_TIME_FLOAT'] : microtime( true ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
