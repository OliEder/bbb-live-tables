...
class BBB_Tournament_Bracket {
	...
	public function __construct() {
		...
		add_shortcode( 'bbb_bracket', array( $this, 'render_shortcode' ) );
	...
	public function register_rest_routes(): void {
		register_rest_route(
			...
			array(
				'methods'             => 'GET',
				'callback'            => array( $this, 'rest_get_leagues' ),
	...
	public function rest_get_leagues( WP_REST_Request $request ): WP_REST_Response {
		...
		$leagues = apply_filters( 'bbb_table_liga_options', array(), $type_filter );
	...
	public function render_shortcode( $atts ): string {
		...
		$mode    = in_array( $atts['mode'], array( 'ko', 'playoff' ), true ) ? $atts['mode'] : 'ko';
	...
	private function build_bracket_structure( array $api_data, int $liga_id ): array {
		$rounds    = $api_data['rounds'] ?? array();
		$liga_data = $api_data['liga_data'] ?? array();

		$first_round         = reset( $rounds );
		$first_round_matches = count( $first_round['matches'] ?? array() );
	...
	private function render_bracket_html(
		...
		$rounds = $tournament['rounds'] ?? array();
		if ( empty( $rounds ) ) {
		...
		foreach ( $rounds as $round_nr => $round ) {
			if ( $mode === 'playoff' && $best_of > 1 ) {
				...
				$round_for_render = array_merge( $round, array( 'series' => $series ) );
		...
		if ( $mode === 'playoff' && $best_of > 1 ) {
			$last_series      = $this->group_matches_into_series( $rounds[ $max_existing ]['matches'] ?? array(), $best_of );
			$last_match_count = count( $last_series );
		} else {
			$last_match_count = count( $rounds[ $max_existing ]['matches'] ?? array() );
		}
	...
	private function render_match( array $match, int $highlight_club, bool $show_dates, bool $show_logos, string $round_name = '', int $match_number = 1 ): string {
		...
		if ( $match['abgesagt'] ) {
			$match_class .= ' bbb-match-cancelled';
		}
		...
		if ( $has_result ) {
			$sr_label .= ', Ergebnis ' . str_replace( ':', ' zu ', $match['result'] );
		}
	...
	private function render_team_row( array $team, array $match, string $side, int $highlight_club, bool $show_logos, ?string $event_url ): string {
		...
		$classes = array( 'bbb-team' );
		if ( $is_winner ) {
			$classes[] = 'bbb-team-winner';
		}
		if ( $is_loser ) {
			$classes[] = 'bbb-team-loser';
		}
		if ( $is_bye ) {
			$classes[] = 'bbb-team-bye';
		}
		if ( $is_own ) {
			$classes[] = 'bbb-team-own';
		}

		$result_parts = $match['result'] ? explode( ':', $match['result'] ) : array( null, null );
	...
	private function group_matches_into_series( array $matches, int $best_of ): array {
		...
		foreach ( $matches as $match ) {
			...
			if ( $match['winner'] === 'home' ) {
				$winner_pid = $home_pid;
			} elseif ( $match['winner'] === 'guest' ) {
				$winner_pid = $guest_pid;
			} else {
				continue;
			}

			if ( $winner_pid === min( $ids ) ) {
				++$pairings[ $key ]['wins_a'];
			} else {
				++$pairings[ $key ]['wins_b'];
			}
		...
		foreach ( $pairings as &$series ) {
			...
			if ( $series['wins_a'] >= $wins_needed ) {
				$series['series_winner'] = 'a';
			} elseif ( $series['wins_b'] >= $wins_needed ) {
	...
	private function render_round_playoff( array $round, int $round_nr, int $highlight_club, bool $show_dates, bool $show_logos, int $sub_heading, int $best_of ): string {
		$series_list = $round['series'] ?? array();
	...
	private function render_series_team( array $team, string $side, ?string $winner, int $highlight_club, bool $show_logos ): string {
		...
		$classes = array( 'bbb-series-team' );
		if ( $is_winner ) {
			$classes[] = 'bbb-series-team-winner';
		}
		if ( $is_loser ) {
			$classes[] = 'bbb-series-team-loser';
		}
		if ( $is_own ) {
			$classes[] = 'bbb-team-own';
		}
		if ( $side === 'b' ) {
			$classes[] = 'bbb-series-team-right';
		}
		...
		if ( $is_winner ) {
			$html .= '<span class="bbb-sr-only"> (Serien-Gewinner)</span>';
		} elseif ( $is_loser ) {
			$html .= '<span class="bbb-sr-only"> (Ausgeschieden)</span>';
		}
		$html .= '</div>';
